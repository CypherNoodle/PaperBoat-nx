...
Runtime messages use the port's regular `switch/paperboat/logs/Paperboat.log`.
The engine logger is synchronous on Switch so messages are not left queued on a
logging thread when the process exits unexpectedly.
