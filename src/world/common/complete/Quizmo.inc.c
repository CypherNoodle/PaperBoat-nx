...
API_CALLABLE(N(Quizmo_CreateStage)) {
    ...
    if (isInitialCall) {
        // Extract to locals: evaluation order is mangled
        s32 quizOriginX = evt_get_variable(script, QUIZ_ARRAY_ORIGIN_X);
        s32 quizOriginY = evt_get_variable(script, QUIZ_ARRAY_ORIGIN_Y);
        s32 quizOriginZ = evt_get_variable(script, QUIZ_ARRAY_ORIGIN_Z);
        N(Quizmo_StageEffect) = fx_quizmo_stage(0, quizOriginX, quizOriginY, quizOriginZ);
        N(Quizmo_AudienceEffect) = fx_quizmo_audience(0, quizOriginX, quizOriginY, quizOriginZ);
        N(Quizmo_VannaTEffect) = fx_quizmo_assistant(0, quizOriginX, quizOriginY, quizOriginZ, 1.0f, 0);
